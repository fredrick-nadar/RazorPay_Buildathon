"""Seed 500+ live records into Razorpay Test Mode API with rate-limiting and verify retrieval."""
...
def post_json_with_retry(url: str, auth_header: str, payload: dict, max_retries: int = 5) -> dict | None:
...
def main() -> None:
    ...
    auth_header = "Basic " + base64.b64encode(
        f"{client.key_id}:{client.key_secret}".encode()
    ).decode()

    target_count = 500
    print(f"[*] Generating {target_count} records via POST https://api.razorpay.com/v1/orders ...")
    ...
    def create_single_order(idx: int) -> str | None:
        ...
        res = post_json_with_retry("https://api.razorpay.com/v1/orders", auth_header, payload)
        if res:
            return res.get("id")
        return None
    ...
    with ThreadPoolExecutor(max_workers=8) as executor:
        futures = [executor.submit(create_single_order, i + 1) for i in range(target_count)]
        for i, fut in enumerate(as_completed(futures), start=1):
            ...
            if i % 50 == 0 or i == target_count:
                ...
                print(f"    -> Progress: {i}/{target_count} processed ({len(created_ids)} succeeded, {failed_count} failed) in {elapsed:.1f}s ({len(created_ids)/elapsed:.1f} req/s)")
    ...
    while True:
        ...
        try:
            with urllib.request.urlopen(req, timeout=20) as resp:
                ...
                print(f"    Page fetched (skip={skip:03d}): {len(items)} items | Cumulative: {len(fetched_orders)}")
    ...
    if fetched_orders:
        ...
        print(f" Sample Amount (Paise)                 : {sample.get('amount')} (INR {sample.get('amount', 0)/100:.2f})")
